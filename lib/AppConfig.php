...
class AppConfig {
    ...
    public function SetBoshUrl($boshUrl)
    {
        ...
        if (strlen($boshUrl) > 0 && !preg_match("/^https?:\/\//i", $boshUrl)) $boshUrl = "http://" . $boshUrl;
        $this->logger->info("SetBoshUrl: " . $boshUrl, array("app" => $this->appName));
    ...
    public function GetBoshUrl()
    {
        ...
        if (empty($val)) $val = $this->predefBoshUrl;
        return $val;
    }
